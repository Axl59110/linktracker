...
@section('title', 'Nouvelle commande - Link Tracker')

@section('breadcrumb')
    <a href="{{ route('orders.index') }}" class="text-brand-500 hover:text-brand-600">Commandes</a>
    <span class="mx-2 text-neutral-400">/</span>
    <span class="text-neutral-900 font-medium">Nouvelle commande</span>
@endsection

@section('content')
    <x-page-header title="Nouvelle commande" subtitle="Planifiez un achat de backlink" />
            ...
            <form action="{{ route('orders.store') }}" method="POST" class="space-y-5">
                ...
                <div>
                    ...
                    <input type="url" id="target_url" name="target_url" value="{{ old('target_url') }}"
                        ...
                        class="block w-full px-4 py-2 border border-neutral-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-brand-500" required />
                </div>

                {{-- Ancre --}}
                <div>
                    <label for="anchor_text" class="block text-sm font-medium text-neutral-700 mb-1">Texte d'ancre</label>
                    <input type="text" id="anchor_text" name="anchor_text" value="{{ old('anchor_text') }}"
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <select id="tier_level" name="tier_level" required
                            class="block w-full px-4 py-2 border border-neutral-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-brand-500">
                            <option value="tier1" {{ old('tier_level', 'tier1') === 'tier1' ? 'selected' : '' }}>Tier 1</option>
                            <option value="tier2" {{ old('tier_level') === 'tier2' ? 'selected' : '' }}>Tier 2</option>
                        </select>
                    </div>
                    <div>
                        ...
                        <select id="spot_type" name="spot_type" required
                            class="block w-full px-4 py-2 border border-neutral-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-brand-500">
                            <option value="external" {{ old('spot_type', 'external') === 'external' ? 'selected' : '' }}>Externe</option>
                            <option value="internal" {{ old('spot_type') === 'internal' ? 'selected' : '' }}>Interne (PBN)</option>
                        </select>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        <label for="ordered_at" class="block text-sm font-medium text-neutral-700 mb-1">Date de commande</label>
                        <input type="date" id="ordered_at" name="ordered_at" value="{{ old('ordered_at', now()->format('Y-m-d')) }}"
                            class="block w-full px-4 py-2 border border-neutral-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-brand-500" />
                    </div>
                    <div>
                        <label for="expected_at" class="block text-sm font-medium text-neutral-700 mb-1">Date de publication prévue</label>
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        <label for="contact_name" class="block text-sm font-medium text-neutral-700 mb-1">Nom du contact</label>
                        <input type="text" id="contact_name" name="contact_name" value="{{ old('contact_name') }}"
                            class="block w-full px-4 py-2 border border-neutral-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-brand-500" />
                    </div>
                    <div>
                        <label for="contact_email" class="block text-sm font-medium text-neutral-700 mb-1">Email du contact</label>
                ...
                <div>
                    ...
                    <textarea id="notes" name="notes" rows="3"
                        class="block w-full px-4 py-2 border border-neutral-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-brand-500">{{ old('notes') }}</textarea>
                </div>

                <div class="flex items-center gap-3 pt-2">
                    <x-button variant="primary" type="submit">Créer la commande</x-button>
                    <x-button variant="secondary" href="{{ route('orders.index') }}">Annuler</x-button>
                </div>
            </form>
        </div>
    </div>
